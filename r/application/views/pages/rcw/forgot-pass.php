...
<body>
		...
		<!-- Login Form -->
		<form class="login-f" id="ForgotForm">
			<!-- Title -->
			<div class="title-from">Forgot Password</div>
			<!-- Username -->
			<input type="email" placeholder="Enter Email" name="email" class="user-l">
			<div class="remember-line">
			</div>
			<!-- Login -->
			<input type="button" onclick="onConfirm()" class="login-btn" value="Submit">
		</form>
		<div class="line-op">
			<!-- Sign in -->
			<div class="reg-p">
	...
	<script>
		...
		var baseURL="<?php echo base_url();?>";
			function onSubmit(){
				$.ajax({
					 url:baseURL+'index.php/Rcw_controller/changeForgotPass',
					 method: 'post',
					 data: $("#ForgotForm").serialize(),
					 success: function(output, status, xhr){
						 if(output=='1')
							 sAlert("Password Reset Successful, Check your Email!", './login');
						 else
							fAlert(output);
					 },
			...
			function onConfirm(){
				...
				$.ajax({
					 url:baseURL+'index.php/Rcw_controller/forgotPass',
					 method: 'post',
					 data: $("#ForgotForm").serialize()+ '&code=' + code,
					 success: function(output, status, xhr){
						 if(output=="1"){
